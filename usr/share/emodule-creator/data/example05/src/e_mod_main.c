...
static void _skeletor_radio_cb_cfg(void *data, E_Menu *mn, E_Menu_Item *mi);
...
_skeletor_menu_cb_cfg(void *data, E_Menu *menu __UNUSED__, E_Menu_Item *mi __UNUSED__)
{
    ...
    e_int_config_skeletor_module(con, inst->cfg);
}
...
_skeletor_cb_mouse_down(void *data, Evas *e __UNUSED__, Evas_Object *obj __UNUSED__, void *event_info)
{
    ...
    else if ((ev->button == 3) && (!inst->menu))
    {
        ...
        mi = e_menu_item_new(m);
        e_menu_item_label_set(mi,"Submenu");
        e_menu_item_submenu_set(mi,sm);

        mi = e_menu_item_new(sm);
        e_menu_item_label_set(mi,"Sub_Item");
        e_util_menu_item_theme_icon_set(mi, "configure");
        e_menu_item_callback_set(mi, _skeletor_menu_cb_cfg, inst);
        ...
        mi = e_menu_item_new(sm);
        e_menu_item_label_set(mi, "radio button 1");
        e_menu_item_radio_set(mi, 1);
        e_menu_item_radio_group_set(mi, 1);
        e_menu_item_callback_set(mi, _skeletor_radio_cb_cfg, inst);

        /*Maybe there is a better way to do this...*/
        if(skeletor_config->radio_val == 1)
            e_menu_item_toggle_set(mi, 1);
        else if(skeletor_config->radio_val == 0)
            e_menu_item_toggle_set(mi, 1);

        mi = e_menu_item_new(sm);
        e_menu_item_label_set(mi, "radio button 2");
        e_menu_item_radio_set(mi, 1);
        e_menu_item_radio_group_set(mi, 1);
        e_menu_item_callback_set(mi, _skeletor_radio_cb_cfg, inst);

        if(skeletor_config->radio_val == 2)
            e_menu_item_toggle_set(mi, 1);
...
static void
_skeletor_radio_cb_cfg(void *data __UNUSED__, E_Menu *mn __UNUSED__, E_Menu_Item *mi)
{
    if(!skeletor_config) return;

    if(strcmp(mi->label,"radio button 1") == 0)
        skeletor_config->radio_val = 1;
    else
        skeletor_config->radio_val = 2;
}
/*
